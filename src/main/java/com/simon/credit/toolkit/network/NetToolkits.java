...
public class NetToolkits {
	...
	public static String filterLocalHost(String host) {
		...
		if (host.contains("://")) {
			...
			if (NetToolkits.isInvalidLocalHost(u.getHost())) {
				return u.setHost(NetToolkits.getLocalHost()).toFullString();
			}
		} else if (host.contains(":")) {
			int i = host.lastIndexOf(':');
			if (NetToolkits.isInvalidLocalHost(host.substring(0, i))) {
				return NetToolkits.getLocalHost() + host.substring(i);
			}
		} else {
			if (NetToolkits.isInvalidLocalHost(host)) {
				return NetToolkits.getLocalHost();
			}
		}
		return host;
	}
	...
	private static InetAddress parseInetAddress() throws SocketException {
