...
public final class QuickSort {
	...
	public static final void sort(int[] array, int left, int right) {
		...
		// while循环的目的: 比pivot值小的放到左边, 比pivot值大的放到右边
		while (leftIndex < rightIndex) {
			// 在pivot的左边一直找, 直至找到大于等于pivot值, 才退出
			while (array[leftIndex] < pivot) {
				leftIndex++;
			}

			// 在pivot的右边一直找, 直至找到小于等于pivot值, 才退出
			...
			// 如果交换完后, 发现这个array[leftIndex]==pivot, 则rightIndex--, 前移
			if (array[leftIndex] == pivot) {
				rightIndex--;
			}
			// 如果交换完后, 发现这个array[rightIndex]==pivot, 则leftIndex++, 后移
